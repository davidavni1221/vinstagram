<template><section class="user-page-container flex-column">
  <div class="info-prfile-page flex">
    ...
    <div class="flex post-follo regular"><p><span class="bold">{{user.storys.length}}&nbsp;</span>post</p><p>
      <span class="bold">{{user.followers.length+265}}&nbsp;</span>followers</p><p><span class="bold">
        {{user.following.length+365}}&nbsp;</span>following</p></div>
...
<svg aria-label="" class="_ab6-" color="#262626" fill="#262626" height="12" role="img" viewBox="0 0 24 24" width="12"><polygon fill="none" points="20 21 12 13.44 4 21 4 3 20 3 20 21" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"></polygon></svg>
  ...
  <div class="img-user hover" v-for ="story in user.storys" :key="story._id">
...
data() {
    return {
     ...
     created() {
      if (this.$route.query.plan==="saved") {
        this.posts=false
      }
