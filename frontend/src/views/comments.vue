...
<div @click="toStory" class="close-deteils-conteiner"><svg aria-label="Close" class="fg7vo5n6 lrzqjn8y" color="#ffffff" fill="#ffffff" height="18" role="img" viewBox="0 0 48 48" width="18"><title>Close</title><path clip-rule="evenodd" d="M41.8 9.8L27.5 24l14.2 14.2c.6.6.6 1.5 0 2.1l-1.4 1.4c-.6.6-1.5.6-2.1 0L24 27.5 9.8 41.8c-.6.6-1.5.6-2.1 0l-1.4-1.4c-.6-.6-.6-1.5 0-2.1L20.5 24 6.2 9.8c-.6-.6-.6-1.5 0-2.1l1.4-1.4c.6-.6 1.5-.6 2.1 0L24 20.5 38.3 6.2c.6-.6 1.5-.6 2.1 0l1.4 1.4c.6.6.6 1.6 0 2.2z" fill-rule="evenodd"></path></svg></div>
  <div class="comment-modal flex">
    
        <!-- <pre>{{story}}</pre> -->
       <div class="img-conteiner" img-><img class="img-deteils" :src="story.imgUrl"></div>
    
    <div class="flex-culomn info-deteils bgc-deteils">
      <div>
        <div class="top-info-details"><div class="flex">
        ...
        <button @click="removeStory(story._id)" class="info-button bgc-deteils"><svg aria-label="More options" class="_ab6- option" color="#262626" fill="#262626" height="24" role="img" viewBox="0 0 24 24" width="24"><circle cx="12" cy="12" r="1.5"></circle><circle cx="6" cy="12" r="1.5"></circle><circle cx="18" cy="12" r="1.5"></circle></svg></button>
        </div>
                  <div class="flex story-discription-deteils" v-if="story.txt"> <img class="profile-pic" :src="story.by.imgUrl"><p class="bold">{{story.by.fullname}}</p><p class="regular">&nbsp;{{story.txt}}</p></div>

        <div class="flex comments-deteils" v-for="comment in story.comments" :key="comment._id">
          
          <div class="flex"><img class="profile-pic-deteils" :src="comment.by.imgUrl" >
       <div class="name-comment-date-details flex-column"> <div class="flex"><p class="bold">{{ comment.by.fullname }}&nbsp;</p>
         <p class="regular"> {{ comment.txt }}</p></div>
         <p class="bold-12-regular-grey">{{ comment.createdAt }}</p></div></div>
...
<div class="right-icon">
  <svg aria-label="Save" class="_ab6-" color="#262626" fill="#262626" height="24" role="img" viewBox="0 0 24 24" width="24"><polygon fill="none" points="20 21 12 13.44 4 21 4 3 20 3 20 21" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"></polygon></svg>  </div>
...
<p class="date">{{ story.createdAt }}</p>
          <div class="add-comment ">
            ...
            <button @click="postComment(story)" class="btn-post" :class="{ typing: isActive }">Post</button></div>
          </div>
    </div>
